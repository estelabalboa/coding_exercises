...
select cast(sum(case when is_low_fat_flg = 1 and is_recyclable_flg = 1
                then 1 else 0 end) as double) / cast(count(product_id) as double ) as pct_low_fat_and_recyclable 
from products